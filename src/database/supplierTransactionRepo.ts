...
export const getTransactionsForSupplier = async (
    ...
    const rows = await db.getAllAsync<SupplierTransaction>(
        `SELECT id, supplier_id, user_id, type, amount, note, date
...
export const addSupplierTransaction = async (
    ...
    await db.runAsync(
        `INSERT INTO supplier_transactions (supplier_id, user_id, type, amount, note, date)
     VALUES (?, ?, ?, ?, ?, ?)`,
        [supplierId, userId, type, amount, note, date]
    );
...
export const deleteSupplierTransaction = async (
    ...
    await db.runAsync(`DELETE FROM supplier_transactions WHERE id = ? AND user_id = ?`, [
        transactionId,
        userId,
    ]);
